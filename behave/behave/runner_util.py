...
def parse_features(feature_files, language=None):
    ...
    features = []
    for domain in feature_files:    #传入多个功能域,遍历
        from behave.sql import getFeature    #导出获得案例
        feature_files = getFeature(domain)    #返回查询的功能域
        for location in feature_files:    #遍历feature
            #by guohongjie 不知道这些是干什么的,注释掉
            ...
            #注释结束
            feature = parser.parse_feature_scenario_step(location, language=language)    #传入feature名称,拼接执行脚本
            # feature = parser.parse_file(filename, language=language)   #by guohongjie 放弃解析文件
            if feature:
                # -- VALID FEATURE:
                # SKIP CORNER-CASE: Feature file without any feature(s).
                scenario_collector.feature = feature
                # scenario_collector.add_location(feature)
            # -- FINALLY:
